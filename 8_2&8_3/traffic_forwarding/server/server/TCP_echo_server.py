...
def break_all_client():
    ...
    for user in user_list:
        try:
            ...
            user_writer.close()
            logging.info('已断开用户连接:', user_list[user]['addr'])
...
async def handle_echo(reader, writer):
    ...
    ident = json.loads(ident.decode())
    if ident['other'] == 'hello':
        writer.write('等待服务器配置规则...'.encode())
        await writer.drain()
    try:
    ...
    try:
        ...
        if find_dest:
            ...
            while True:
                ...
                try:
                    re_msg = await s_reader.read(5120)
                    logging.info('已收到' + str(server_addr) + '给' + str(client_addr) + '的回复\n')
                    # print('已收到' + str(server_addr) + '给' + str(client_addr) + '的回复：\n' + re_msg.decode())
                    try:
                        writer.write(re_msg)
                        await writer.drain()
